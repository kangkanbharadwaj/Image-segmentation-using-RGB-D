...
imgDir = glob.glob("/home/bharadwaj/ImageSegmentation/data/streetObjects2Data/test/*.jpg")
net = caffe.Net('/home/bharadwaj/ImageSegmentation/data/streetObjects2Data/fuseResNet34_org.prototxt', '/home/bharadwaj/ImageSegmentation/data/streetObjects2Data/snapshots/snapshot_iter_' + str(iteration) + '.caffemodel', caffe.TEST)

for images in sorted(imgDir, key=stringSplitByNumbers):
    
    img = cv2.imread (images)    
    imageName = images.split('/')[7].split('.')[0]
    depth = cv2.imread("/home/bharadwaj/ImageSegmentation/data/streetObjects2Data/test/%s.exr" %(imageName), cv2.IMREAD_ANYDEPTH | cv2.IMREAD_ANYCOLOR)    
