...
from DDNet import *
from UNet import *
from Res34_enc_DDNet_dec import *
from DPDB_enc_Unet_dec import *
...
def train_validate(Model=None,N_Class=0,imgh=0,imgw=0,croph=0,cropw=0,c_blc=None,c_blc_T=None,opt=None,LR=0,decay=0.0,Momentum=0.95,epsilon=0.0,beta1=0.0,epoch=100,
    ...
    if Model == 'ResNet':
        print ("Using ResNet for segmentation")
        pred_score = ResNet50(X,training,N_Class)   
    elif Model == 'UNet':
        print ("Using UNet for segmentation")
        pred_score = make_unet(X,training,N_Class)   
    elif Model == 'DDNET':
        print ("Using DDNet for segmentation")
    ...
    elif Model == 'DPDB_UNet':
        print ("Clubbing DPDB Encoder and UNet decoder for segmentation")
        pred_score = DPDB_encoder_UNet_decoder(X,training,N_Class) 
    elif Model == 'ResNet_DDNet':
        print ("Using ResNet-34 encoder and DDNet decoder for segmentation")
        Reshaped_map_decoder1, Reshaped_map_decoder2, pred_score = Res34_encoder_DDNet_decoder(X, training,N_Class)
    ...
    if Model == 'ResNet':
        print ("\ncomputing loss from ResNet")
        Totalloss = cross_entropy_loss(segMap, Reshaped_labels, N_Class,class_weights)
    if Model == 'UNet':
        print ("\nComputing loss from UNet")
        Totalloss = cross_entropy_loss(segMap, Reshaped_labels, N_Class,class_weights)    
    if Model == 'DPDB_UNet':
        print ("\nComputing loss from DPDB+UNet model")
        Totalloss = cross_entropy_loss(segMap, Reshaped_labels, N_Class,class_weights)
    if Model == 'DDNET':
        print ("\nComputing loss from DDNet")
    ...
    if Model == 'ResNet_DDNet':
        print ("\nComputing loss from ResNet+DDNet")
        Loss_decoder1 = cross_entropy_loss(Reshaped_map_decoder1, Reshaped_labels, N_Class,class_weights)
        Loss_decoder2 = cross_entropy_loss(Reshaped_map_decoder2, Reshaped_labels, N_Class,class_weights)
        Loss_final_decoder = cross_entropy_loss(segMap, Reshaped_labels, N_Class,class_weights)
        Totalloss = Loss_decoder1 + Loss_decoder2 + Loss_final_decoder 
    ...
    print ("\n---------------------------------------------- Accumulate summaries and merge them to display in tensorboard ------------------------------------------------------------------")    
    tf.summary.histogram("Softmax activation on predicted image", softmax)    
    tf.summary.image("Image to train", _train_image, max_outputs=2)
    tf.summary.image("Label to train", _train_label, max_outputs=2)     
    if augmentation == 'True':
        tf.summary.image("Augmented image to train", train_image, max_outputs=2)        
        tf.summary.image("Augmented label to train", train_label, max_outputs=2)
        if crop == 'True':
            tf.summary.image("Predicted mask", tf.cast(tf.reshape((tf.argmax(softmax,1)), [1,croph,cropw,1]), tf.uint8), max_outputs=2) 
        else:
            tf.summary.image("Predicted mask", tf.cast(tf.reshape((tf.argmax(softmax,1)), [1,imgh,imgw,1]), tf.uint8), max_outputs=2) 
    else:
        tf.summary.image("Predicted mask", tf.cast(tf.reshape((tf.argmax(softmax,1)), [1,imgh,imgw,1]), tf.uint8), max_outputs=2) 
    tf.summary.scalar('Cross entropy loss', Totalloss) 
    tf.summary.scalar('Accuracy', accuracy)
    tf.summary.scalar('Intersection Over Union', IOU)
    ...
    with tf.Session(config=config) as sess:
        try:
            ...
            # Get number of parameters used in the network
            parameter_count = sess.run(parameter_count)
            print ("\nParameters used in the network are: %d" %(parameter_count))
            ...
            for i in range(init_iterations, MX_ITR):
                ...
                c_loss,train_step,acc,iou,_,step_summary,global_step_value = sess.run([Totalloss,optimizer,accuracy,IOU,IOU_update,summary_op,global_step], 
                ...
                if i > 0 and i % brkpt == 0:
                    ...
                    # Validate on snmall patches if trained on small patches
                    if crop == 'True':                    
                        ...
                        for n in range (len(valset)):                        
                            ...
                            while p+256 <= height:
                                ...
                                while j+256 <= width:                                                                
                                    ...
                                    valloss,valacc,valiou = sess.run([Totalloss,accuracy], feed_dict={X:valImg, y:valLabel, training: False})    
                                    print ("Validating on image %s \t patch: %d \t CE Loss: %.3f \t Accuracy: %.2f" %(valset[n].split('/')[len(valset[n].split('/'))-1],patch,valloss,valacc))                                                                               
                        ...
                        for n in range (len(valset)):                        
                            ...
                            valloss,valacc = sess.run([Totalloss,accuracy], feed_dict={X:valimg, y:vallbl, training: False})    
                            print ("Validating on image %s \t  CE Loss: %.3f \t Accuracy: %.2f" %(valset[n].split('/')[len(valset[n].split('/'))-1],valloss,valacc))                                                                               
